...
def coder(string, img):
    im = Image.open(img)
    binaryStringMap = map(bin, bytearray(string))
    listOfImageData = list(im.getdata())
    #print 'Before:',listOfImageData
...
def decoder(img):
    ...
    for element in dataList:
        ...
        counter = counter + 1
        if counter > 6:         
            letter = str(unichr(int(binword, 2)))
...
if __name__ == '__main__':
    #coder('hah', 'bitmapa.bmp')
    #decoder('codedIMG.bmp')
    if len(argv) == 4:
        if argv[1] == '-c':
            coder(argv[3], argv[2])
    elif len(argv) == 3:
